...
export function isBoardValid(board: Board) {
	if (board.some(sq => (!sq.superPos || sq.superPos.length === 0) && sq.value === 0)) return false
	return checkIncompleteRows(board) && checkIncompleteCols(board) && checkIncompleteBoxes(board)
}
