...
export function Board({
	...
	return (
		<div className='p-[5px] w-[100%] flex flex-col items-center justify-center gap-[15px] relative'>
			<div className='debug p-[2px] absolute top-[-50px] right-0' onClick={solve}>
				solve
			</div>
			...
			<div className='grid grid-cols-9 border-[3px] sm:w-[450px] border-black rounded-[5px] shadow-board w-[100%] relative'>
				{board.map(sq => {
					return (
						<Square sq={sq} key={sq.id} select={selectSquare} selected={selectedSquare?.id === sq.id} isInitial={initialSquares?.includes(sq.id)} />
					)
				})}
			</div>
		</div>
